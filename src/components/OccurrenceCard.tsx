  ...
  return (
    ...
    >
      ...
      <div className="pt-3 mt-2.5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
        ...
        {/* Ações Exclusivas COBOM (Editar / Excluir) */}
        {isCobom && (
          <div className="flex items-center gap-1.5">
            {onEdit && (
              <button
                ...
                className="px-2 py-1 text-slate-700 hover:text-slate-950 hover:bg-slate-100 border border-slate-300 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
                title="Editar dados da ocorrência (COBOM)"
              >
            ...
            {onDelete && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (confirm(`Atenção COBOM: Deseja realmente excluir a ocorrência ${occurrence.protocol}?`)) {
                    onDelete(occurrence.id);
                  }
                }}
                className="px-2 py-1 text-rose-700 hover:text-rose-900 hover:bg-rose-50 border border-rose-200 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
